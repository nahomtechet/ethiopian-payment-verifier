...
export class AwashParser extends BaseParser {
  readonly providerName = 'awash';

  matches(input: string): boolean {
    return (
      /awash/i.test(input) || 
      /awashpay/i.test(input)
    );
  }

  parseSMS(smsText: string): ParseResult {
    // Awash Bank Transaction ID pattern
  ...
  async verifyOnline(input: string, options: VerifierOptions = {}): Promise<VerificationResult> {
    ...
    if (input.startsWith('http://') || input.startsWith('https://')) {
      url = input;
      const match = input.match(/\/receipt\/([^/?#]+)/i) || input.match(/[?&]id=([^&]+)/i) || input.match(/\/([A-Z0-9\-]+)$/i);
      transactionId = match ? match[1] : 'URL_TXN';
    } else {
      transactionId = input.trim();
      url = `https://awashpay.awashbank.com:8225/receipt/${transactionId}`;
    }
